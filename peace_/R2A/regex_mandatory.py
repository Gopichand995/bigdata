...
rule_query = "output[Branch] != '' AND output[Cost Center] != '' OR output[Credit] != '' WHERE ( output[Credit] == 10 AND STARTSWITH(output[Branch], 'B') AND TYPE(output[JE Line description]) == ''char'')"
function_mapping = {"": "Operator", "STARTSWITH": "Start With", "ENDSWITH": "End With", "LEN": "Length",
                    "SUM": "Sum", "MAX": "Max", "MIN": "Min", "AVG": "Avg", "TYPE": "Type"}
...
if 'WHERE' in rule_query.split():
    ...
    condition_creator_query = condition_creator_query.replace(' ', '')[1:len(condition_creator_query) - 1]
    ...
    condition_operator_list = list(map(
        lambda x: "" if 'TYPE' in x else "" if re.search("(\!=|==|=|[<>]=?|<>).*", x) is None else re.search(
            "(\!=|==|=|[<>]=?|<>).*", x).group(1), condition_creator_query_partition))
    print("condition_operator_list: ", condition_operator_list)

    condition_output_char_list = list(map(lambda x: combined_compare_value(x), condition_creator_query_partition))
    print("condition_output_char_list: ", condition_output_char_list)

    condition_output_char_type_list = list(map(lambda x: combined_compare_value_type(x), condition_output_char_list))
